  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/login" && method === "POST") {
        ...
        if (!email || !senha) return json({ error: "email e senha obrigatórios" }, 400, env);

        const user = await env.DB.prepare("SELECT id, senha_hash FROM usuarios WHERE email = ?")
          .bind(email).first();
        if (!user) return json({ error: "credenciais inválidas" }, 401, env);

        const ok = await verificarSenha(senha, user.senha_hash);
        if (!ok) return json({ error: "credenciais inválidas" }, 401, env);
        ...
        return json({ token, expira_em: expira }, 200, env);
      }
      ...
      if (path === "/api/posts" && method === "GET") {
        ...
        return json({ posts: results }, 200, env);
      }
      ...
      if (matchSlug && method === "GET") {
        ...
        if (!post) return json({ error: "não encontrado" }, 404, env);
        return json({ post }, 200, env);
      }
      ...
      if (matchComentarios && method === "GET") {
        ...
        return json({ comentarios: results }, 200, env);
      }
      ...
      if (path.startsWith("/api/admin/")) {
        ...
        if (path === "/api/admin/posts" && method === "POST") {
          ...
          if (!body.titulo || !slug) return json({ error: "título obrigatório" }, 400, env);
          ...
          return json({ ok: true, id: r.meta.last_row_id, slug }, 201, env);
        }
        ...
        if (matchAdminPost && method === "PUT") {
          ...
          if (!existente) return json({ error: "não encontrado" }, 404, env);
        ...
        if (path === "/api/admin/upload" && method === "POST") {
          ...
          return json({ ok: true, url: publicUrl }, 201, env);
        }
        ...
        if (matchComentarioAdmin && method === "PUT") {
          ...
          if (!["aprovado", "rejeitado"].includes(status)) {
            return json({ error: "status inválido" }, 400, env);
          }
      ...
      return json({ error: "erro interno", detalhe: String(err) }, 500, env);
    }
  },
};
